...
####### Sandwich variance estimation functions

### by Yunji Zhou

### Average treatment effect by IPW (with/without trimming)
ATE <- function(y, z, X, DR=FALSE, X.out=NA, alpha=0){
...
ATO <- function(y, z, X, DR=FALSE, X.out=NA){
  ...
  if(DR==FALSE){
    ...
    A.22 <- mean(z/e.h*g.h) # scalar 
    A.33 <- mean((1-z)/(1-e.h)*g.h) # scalar
    A.21 <- -apply(z*( d.g.h -(1-e.h)*g.h*V)/e.h*(y-mu1.h),2,mean) # vector of length v
    A.31 <- -apply((1-z)*(d.g.h+e.h*g.h*V)/(1-e.h)*(y-mu0.h), 2, mean) # vector of length v
    ...
    psi <- cbind((z-e.h)*V, z*(g.h/e.h)*(y-mu1.h), (1-z)*(g.h/(1-e.h))*(y-mu0.h))
    B <- crossprod(psi) / n
    ...
    W1.h <- (g.h/e.h) / sum(g.h/e.h)
    W0.h <- (g.h/(1-e.h)) / sum(g.h/(1-e.h))
    W5.h <- ( z*g.h/e.h+(1-z)*g.h/(1-e.h) ) / sum(z*g.h/e.h+(1-z)*g.h/(1-e.h))
    
    tau <- sum(z*W1.h*(y-m1.h)) - sum((1-z)*W0.h*(y-m0.h)) + sum( W5.h*(m1.h-m0.h))  
    
    # sandwich variance estimator
    A.11 <- crossprod(sqrt(e.h*(1-e.h)) * V) / n  #(v by v matrix)
    A.22 <- crossprod(sqrt(z) * W) / n # W by W matrix
    A.33 <- crossprod(sqrt(1-z) * W) / n # W by W matrix
    A.41 <- - apply( z*(d.g.h - (1-e.h)*g.h*V)/e.h*(m1.h-m0.h- (sum(g.h*m1.h)/sum(g.h) - sum(g.h*m0.h)/sum(g.h))  ), 2, mean) # 1 by v vector
    A.42 <- - apply(z*g.h/e.h*W,2,mean) # 1 by w vector
    A.43 <- - A.42 # 1 by w vector
    A.44 <- A.55 <- mean(z*g.h/e.h)  # scalar
    A.51 <- - apply(z*(d.g.h-(1-e.h)*g.h*V)/e.h*(y-m1.h-mu1.h), 2, mean) # 1 by w vector
    A.52 <- apply(z*g.h/e.h*W,2,mean) # 1 by w vector
    A.61 <- - apply((1-z)*(d.g.h+e.h*g.h*V)/(1-e.h)*(y-m0.h-mu0.h), 2, mean) # 1 by w vector
    ...
    A.col.4 <- c(rep(0, nrow(A.22)+nrow(A.11)+nrow(A.33)),A.44,rep(0,2))
    A.col.5 <- c(rep(0, nrow(A.22)+nrow(A.11)+nrow(A.33)+1),A.55,rep(0,1))
    A.col.6 <- c(rep(0, nrow(A.22)+nrow(A.11)+nrow(A.33)+1+1),A.66)
    
    A <- cbind(A.col.1,A.col.2,A.col.3,A.col.4,A.col.5,A.col.6)
    A.inv <- solve(A)
    A.t.inv <- solve(t(A))  
    
    psi <- cbind((z-e.h)*V, c(z  * (y-W %*% out.trt$coefficients)) * W, c( (1-z) * (y-W %*% out.ctrl$coefficients) )* W, 
                 z*g.h/e.h*(m1.h-m0.h-(sum(g.h*m1.h)/sum(g.h) - sum(g.h*m0.h)/sum(g.h)) ), z*g.h/e.h*(y-m1.h-mu1.h), (1-z)*g.h/(1-e.h)*(y-m0.h-mu0.h))
    B <- crossprod(psi) / n
    
    sigma <- A.inv %*% B %*% A.t.inv
    c <- c(rep(0,ncol(V)+ncol(W)+ncol(W)),1,1,-1)
...
### Entropy weights
ATEN <- function(y, z, X, DR=FALSE, X.out=NA){
  ...
  diff <- function(cov, z, ps){
    ...
    v1 <- cov[z == 1]
    v0 <- cov[z == 0]
    w1 <- -log(ps[z == 1])-(1-ps[z == 1])/ps[z == 1]*log(1-ps[z == 1])
    w0 <- -ps[z == 0]/(1-ps[z == 0])*log(ps[z == 0])-log(1-ps[z == 0])
    n1 <- length(v1)
    n0 <- length(v0)
    delta <- abs(sum(v1*w1) / sum(w1) - sum(v0*w0) / sum(w0))
    # tstat <- delta / sqrt(var(v1)/n1 + var(v0)/n0)
  ...
  d.g.h <- (-log(e.h)+log(1-e.h))  *   e.h*(1-e.h)*V
  
  # point estimate - ATEN
  mu1.h <- sum(z*y*(g.h/e.h)) / sum(z*(g.h/e.h))
  mu0.h <- sum( (1-z)*y* g.h/(1-e.h) ) / sum((1-z)*g.h/(1-e.h))
  tau <- mu1.h - mu0.h
...
### Matching weights
ATM <- function(y, z, X, DR=FALSE, X.out=NA){
  ...
  diff <- function(cov, z, ps){
    ...
    v1 <- cov[z == 1]
    v0 <- cov[z == 0]
    w1 <- pmin( ps[z == 1], 1-ps[z == 1])/ps[z == 1]
    w0 <- pmin(ps[z == 0], 1-ps[z == 0]) /(1-ps[z == 0])
    n1 <- length(v1)
    n0 <- length(v0)
    delta <- abs(sum(v1*w1) / sum(w1) - sum(v0*w0) / sum(w0))
    # tstat <- delta / sqrt(var(v1)/n1 + var(v0)/n0)
  ...
  d.g.h <- matrix(NA, nrow = length(g.h), ncol = ncol(V))
  delta <- 0.002
  left <- e.h< (0.5-delta)
  middle <- e.h >= (0.5-delta) & e.h <= (0.5+delta)
  right <- e.h> (0.5+delta)
  d.g.h[left,] <-  1  *   e.h[left]*(1-e.h[left])*V[left,]
  d.g.h[right,] <-  -1  *   e.h[right]*(1-e.h[right])*V[right,]
  D <- matrix(c(1, 0.5-delta, (0.5-delta)^2, (0.5-delta)^3,
                0, 1,         2*(0.5-delta), 3*(0.5-delta)^2,
                1, 0.5+delta, (0.5+delta)^2, (0.5+delta)^3,
                0, 1,         2*(0.5+delta), 3*(0.5+delta)^2),
              byrow = T, nrow = 4, ncol = 4)
  a <- c(solve(D) %*% c(1,0,(1-2*delta)/(1+2*delta),-4/(1+2*delta)^2))
  b <- c(solve(D) %*% c((1-2*delta)/(1+2*delta),4/(1+2*delta)^2,1,0))
  g.h[middle] <- t(a) %*% rbind(e.h,e.h^2,e.h^3,e.h^4)[,middle]
  d.g.h[middle,] <- c(t(a) %*% rbind(1,2*e.h,3*e.h^2,4*e.h^3)[,middle]  *   e.h[middle]*(1-e.h[middle]))*V[middle,]
