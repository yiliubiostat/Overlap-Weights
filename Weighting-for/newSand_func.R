...
### ~~~~ Sandwich Variance for doubly-robust ATC and ATT

### by Yunji Zhou

### Average treatment effect by IPW
...
### Average treatment effect on the treated population (by IPW)
ATT <- function(y, z, X, DR=FALSE, X.out=NA){
  ...
  psi <- cbind((z-e.h)*V, c( (1-z)  * (y-W %*% out.ctrl$coefficients)) * W, 
               (z)*(y-m0.h-sum(w1.h*(y-m1.h))), (1-z)*e.h/(1-e.h)*(y-m0.h-sum(w0.h*(y-m0.h))) )
  B <- crossprod(psi) / n
...
### Average treatment effect on the controls population (by IPW)
ATC <- function(y, z, X, DR=FALSE, X.out=NA){
  ...
  if(DR==FALSE){
    ...
    psi <- cbind((z-e.h)*V, c(z  * (y-W %*% out.trt$coefficients)) * W, 
                 z/e.h*(1-e.h)*(y-m1.h-sum(w1.h*(y-m1.h))), (1-z)*(y-m1.h-sum(w0.h*(y-m0.h))))
    B <- crossprod(psi) / n
