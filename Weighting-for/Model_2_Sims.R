...
md2.alpha0 <- -2.17
weight.data.reps <- PS.model.reps(alpha0 = md2.alpha0, alpha1)
...
sim2.wt.c <- sim.wt.c %>% filter(Row.Num %in% R)
sim2.wt.h <- sim.wt.h %>% filter(Row.Num %in% R)

sim2.aug.c.cc <- sim.aug.c.cc %>% filter(Row.Num %in% R)
sim2.aug.h.cc <- sim.aug.h.cc %>% filter(Row.Num %in% R)

sim2.aug.c.cm <- sim.aug.c.cm %>% filter(Row.Num %in% R)
sim2.aug.h.cm <- sim.aug.h.cm %>% filter(Row.Num %in% R)

sim2.aug.c.mc <- sim.aug.c.mc %>% filter(Row.Num %in% R)
sim2.aug.h.mc <- sim.aug.h.mc %>% filter(Row.Num %in% R)

sim2.aug.c.mm <- sim.aug.c.mm %>% filter(Row.Num %in% R)
sim2.aug.h.mm <- sim.aug.h.mm %>% filter(Row.Num %in% R)


# Design module
weight.data.reps2 <- weight.data.reps %>% filter(Ite %in% R)

# --- Propensity score plot will be draw in SumStat file
# --- p
sim2.p <- length(weight.data.reps2$Z[weight.data.reps2$Z == 1])/(M0*N)
...
for(i in R) {
  ...
  data <- weight.data.reps2 %>% filter(Ite == i)
...
sim2.ratio <- mean(ps.var.trt/ps.var.ctrl)

md2.true.h <- PE.h

# Finally, save data!
save(file = "md2_sims.RData", weight.data.reps2, sim2.p, sim2.ratio, md2.true.h,
     sim2.wt.c, sim2.wt.h, sim2.aug.c.cc, sim2.aug.h.cc, 
     sim2.aug.c.cm, sim2.aug.h.cm, sim2.aug.c.mc, sim2.aug.h.mc, sim2.aug.c.mm, sim2.aug.h.mm )
