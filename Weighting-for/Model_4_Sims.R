...
md4.alpha0 <- 0.98
weight.data.reps <- PS.model.reps(alpha0 = md4.alpha0, alpha1)
...
sim4.wt.c <- sim.wt.c %>% filter(Row.Num %in% R)
sim4.wt.h <- sim.wt.h %>% filter(Row.Num %in% R)

sim4.aug.c.cc <- sim.aug.c.cc %>% filter(Row.Num %in% R)
sim4.aug.h.cc <- sim.aug.h.cc %>% filter(Row.Num %in% R)

sim4.aug.c.cm <- sim.aug.c.cm %>% filter(Row.Num %in% R)
sim4.aug.h.cm <- sim.aug.h.cm %>% filter(Row.Num %in% R)

sim4.aug.c.mc <- sim.aug.c.mc %>% filter(Row.Num %in% R)
sim4.aug.h.mc <- sim.aug.h.mc %>% filter(Row.Num %in% R)

sim4.aug.c.mm <- sim.aug.c.mm %>% filter(Row.Num %in% R)
sim4.aug.h.mm <- sim.aug.h.mm %>% filter(Row.Num %in% R)


# Design module
weight.data.reps4 <- weight.data.reps %>% filter(Ite %in% R)

# --- Propensity score plot will be draw in SumStat file
# --- p
sim4.p <- length(weight.data.reps4$Z[weight.data.reps4$Z == 1])/(M0*N)
...
for(i in R) {
  ...
  data <- weight.data.reps4 %>% filter(Ite == i)
...
sim4.ratio <- mean(ps.var.trt/ps.var.ctrl)

md4.true.h <- PE.h

# Finally, save data!
save(file = "md4_sims.RData", weight.data.reps4, sim4.p, sim4.ratio, md4.true.h,
     sim4.wt.c, sim4.wt.h, sim4.aug.c.cc, sim4.aug.h.cc, 
     sim4.aug.c.cm, sim4.aug.h.cm, sim4.aug.c.mc, sim4.aug.h.mc, sim4.aug.c.mm, sim4.aug.h.mm )
