...
md5.alpha0 <- 1.86
weight.data.reps <- PS.model.reps(alpha0 = md5.alpha0, alpha1)
...
sim5.wt.c <- sim.wt.c %>% filter(Row.Num %in% R)
sim5.wt.h <- sim.wt.h %>% filter(Row.Num %in% R)

sim5.aug.c.cc <- sim.aug.c.cc %>% filter(Row.Num %in% R)
sim5.aug.h.cc <- sim.aug.h.cc %>% filter(Row.Num %in% R)

sim5.aug.c.cm <- sim.aug.c.cm %>% filter(Row.Num %in% R)
sim5.aug.h.cm <- sim.aug.h.cm %>% filter(Row.Num %in% R)

sim5.aug.c.mc <- sim.aug.c.mc %>% filter(Row.Num %in% R)
sim5.aug.h.mc <- sim.aug.h.mc %>% filter(Row.Num %in% R)

sim5.aug.c.mm <- sim.aug.c.mm %>% filter(Row.Num %in% R)
sim5.aug.h.mm <- sim.aug.h.mm %>% filter(Row.Num %in% R)


# Design module
weight.data.reps5 <- weight.data.reps %>% filter(Ite %in% R)

# --- Propensity score plot will be draw in SumStat file
# --- p
sim5.p <- length(weight.data.reps5$Z[weight.data.reps5$Z == 1])/(M0*N)
...
for(i in R) {
  ...
  data <- weight.data.reps5 %>% filter(Ite == i)
...
sim5.ratio <- mean(ps.var.trt/ps.var.ctrl)

md5.true.h <- PE.h

# Finally, save data!
save(file = "md5_sims.RData", weight.data.reps5, sim5.p, sim5.ratio, md5.true.h,
     sim5.wt.c, sim5.wt.h, sim5.aug.c.cc, sim5.aug.h.cc, 
     sim5.aug.c.cm, sim5.aug.h.cm, sim5.aug.c.mc, sim5.aug.h.mc, sim5.aug.c.mm, sim5.aug.h.mm )
